...
void MapIterator::next() {
	...
	positionFirst++;
    while(positionFirst < map.capacity && map.firstHash[positionFirst] == NULL_TELEM) {
        // move in the first array
        positionFirst++;
    }
    if(positionFirst < map.capacity) {
        // if we are within the first array, ..
        return;
    }
    positionSecond++;
    while(positionSecond < map.capacity && map.secondHash[positionSecond] == NULL_TELEM) {
...
TElem MapIterator::getCurrent(){
	...
	if(positionFirst > -1 && positionFirst < map.capacity) {
	    // If we are within the first hashtabelle, ..
        return map.firstHash[positionFirst];
	}
	if(positionSecond > -1 && positionSecond < map.capacity) {
	    // If we are within the second hashtabelle, ..
        return map.secondHash[positionSecond];
	}
...
bool MapIterator::valid() const {
    ...
    return !(map.isEmpty() || positionSecond >= map.capacity);
