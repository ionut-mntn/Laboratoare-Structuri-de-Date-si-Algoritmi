...
/*
worst case = O(n) - am dat de un ciclu sau trebuie resize & rehash
average case = O(1)
best case = O(1), deci Theta(1) - pur si simplu sunt niste calcule, assingment uri
*/
TValue Map::add(TKey c, TValue v) {
    ...
    /* in curs se spune ca: fiecare tabela are mai mult de jumatate din pozitii neocupate, deci fiecare tabela
   are maxim ((m - 1) / 2) pozitii ocupate; deci, in total, trebuie sa fie maxim: 2 * ( m - 1 ) / 2 pozitii ocupate, cumulat, in cele 2 sub-
   */
    // if (len == m - 2) resize_rehash();
    ...
    /* nota: cred ca puteam face un while mare in care sa include si primele 2 verificari din "eviction chain",
    dar m-am incurcat la ceva, asa ca am lasat asa cum se vede mai jos ( la fel si in resize_rehash unde algoritmul este
    foarte similar)*/
            ...
            else                                            // atlfel, continuam cu "eviction chain-ul"
            {
                ...
                { // altfel
                    if (aux == element && poz_start == poz1)    /* Daca am ajuns din nou pe pozitia de la care am plecat, DAR ATENTIE!:
                                                                am ajuns: SI de la pozitia de la care am plecat SI cu acelasi element de inserat => ciclu =>
                                                                */
                        resize_rehash();                        /*  => trebui sa facem resize & rehash */
                }
            }
        }
    }
...
/*
    worst case = average case = best case = Theta(1), deoarece avem doar 2 verificari de facut, niste assignment-uri
*/
...
ca "eviction chain-ul" nu va fi foarte lung, atunci complexitatea este (deci neglijand putin cat de mult timp
costa impingerile dintr-o tabela in alta):
...
//    std::cout << " s a facut resize !!" << '\n';
   ...
   */
    ...
    for (int i = 0; i < lim; i++)
    {
        ...
        if (elem1 != NULL_TELEM)                    /* daca pe pozitia `i` din `ht1` am gasit un element, intr-adevar, atunci: */
        {
            // il adaugam in noua "prima tabela" hash

            int poz1 = h1(elem1.first);             // calculez "hashwert-ul"
            TElem aux = new_ht1[poz1];              // retin intr-o variabila auxiliara elementul curent "in aer"
            new_ht1[poz1] = elem1;                  // inserez elementul de inserat in tabela 1, in locul celui existent deja


            if (aux != NULL_TELEM)                  // daca elementul tocmai inserat a inlocuit un element deja existent si nu valoare nula, atunci:
            {
                while (true)
                {

                    int poz2 = h2(aux.first);       // calculez "hashwert-ul"
                    swap(aux, new_ht2[poz2]);       // inserez elementul de inserat in tabela 2, in locul celui existent deja

                    if (aux != NULL_TELEM)          // daca elementul tocmai inserat a fost inserat pe o pozitie pe care era un element diferit de elementul nul
                    {
                        poz1 = h1(aux.first);       // calculez "hashwert-ul"
                        swap(aux, new_ht1[poz1]);   // inserez elementul de inserat in tabela 1, in locul celui existent deja
                        if (aux == NULL_TELEM)      // daca elementul tocmai inserat a fost inserat pe o pozitie libera
                        {
                            break;                  // parasim "eviction chain ul"
                        }

                    }
                    else                            // altfel (daca elementul tocmai inserat a fost inserat pe o pozitie libera):
                    {
                        break;                      // parasim "eviction chain ul"
                    }
                }
        ...
        if (elem2 != NULL_TELEM)                    /* daca pe pozitia `i` din `ht1` am gasit un element, intr-adevar, atunci: */
        {
            int poz1 = h1(elem2.first);             // calculez "hashwert-ul"
            TElem aux = new_ht1[poz1];              // retin intr-o variabila auxiliara elementul curent "in aer"
            new_ht1[poz1] = elem2;                  // inserez elementul de inserat in tabela 1, in locul celui existent deja
            ...
            if (aux != NULL_TELEM)                  // daca elementul tocmai inserat a inlocuit un element deja existent si nu valoare nula, atunci:
            {
                while (true)
                {
                    ...
                    {
                        poz1 = h1(aux.first);                   // calculez "hashwert-ul"
                        swap(aux, new_ht1[poz1]);               // inserez elementul de inserat in tabela 1, in locul celui existent dejas
                        if (aux == NULL_TELEM)                  // daca elementul tocmai inserat a fost inserat pe o pozitie libera
                        {
                            break;                              // parasesc "eviction chain-ul
                        }
                    }
                    else        // altfel:
                    {
...
Map::~Map() {
    ...
    // dealoc spatiul alocat pentru cele 2 sub-containere
    delete[] ht1;
    delete[] ht2;
}