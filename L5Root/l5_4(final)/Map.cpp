...
//	m = 23; // de dat mai mare aici? vad mai incolo... 
	...
	//m = 31; // este prim si e aproximativ la mijloc, intre 16 si 32
	m = 13;
...
void Map::resize_rehash()
{
	...
	TElem * new_ht1 = new TElem[m * 2]; mark_empty(new_ht1, 2 * m);
	TElem * new_ht2 = new TElem[m * 2]; mark_empty(new_ht2, 2 * m);



	for (int i = 0; i < m; i++)
	{	/* de facut...*/
		TElem e1 = ht1[i], e2 = ht2[i];    // elements to be repositioned

		// Here there is no chance of infinite loops or not enough space.
		if (e1 != NULL_TELEM) {
			// If there is an element, we add it.
			int poz1 = h1(e1.first);       // place in fist hashtabelle
			...
			if (removed != NULL_TELEM) {
				while (true) {
					...
					if (removed == NULL_TELEM) {
						...
						std::swap(removed, new_ht1[poz1]);   // save removed element in first hash
						if (removed == NULL_TELEM) {
							break;
						}
					}
	...
	*/
	...
	if (searchValue != NULL_TVALUE) {
		...
		if (ht1[h1(c)].first == c) {
			...
			oldValue = ht1[h1(c)].second;      // save former value in order to return
			ht1[h1(c)].second = v;             // modify former value
		}
		else {
			// If the key is in the second hashtabelle, ..
			oldValue = ht2[h2(c)].second;     // save former value in order to return
			ht2[h2(c)].second = v;            // modify former value
		}
		return oldValue;
	}
	...
	int poz1 = h1(c);       // calculate where the element should be added
