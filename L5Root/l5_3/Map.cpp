	...
	*/
	...
	if (searchValue != NULL_TVALUE) {
		...
		if (ht1[f1(c)].first == c) {
			...
			oldValue = ht1[f1(c)].second;      // save former value in order to return
			ht1[f1(c)].second = v;             // modify former value
		}
		else {
			// If the key is in the second hashtabelle, ..
			oldValue = ht2[f2(c)].second;     // save former value in order to return
			ht2[f2(c)].second = v;            // modify former value
		}
		return oldValue;
	}
	...
	int poz1 = f1(c);       // calculate where the element should be added
	...
	if (removed == NULL_TELEM) {
		...
		while (true) {
			...
			int poz2 = f2(removed.first);       // Calculate where the removed element should be placed
			...
			if (removed == NULL_TELEM) {
				...
				poz1 = f1(removed.first);          // Calculate where the removed element should be placed
