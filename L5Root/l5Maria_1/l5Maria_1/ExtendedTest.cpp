...
void testIteratorSteps(SortedMap& m, Relation r) {
    ...
    if (li.valid()) {
        elem = li.getCurrent();
        cout << "elem.first = " << elem.first << " " << "elem.second = " << elem.second << '\n';
        //cout << elem.first;
        count++;
        li.next();
    }
...
//    cout<<endl;
//    cout<<li.getCurrent().first; // 1
   while (li.valid()) {
       cout << li.getCurrentNode() << '\n';
       // cout << li.getCurrentNode().info << '\n';
       //cout << li.getCurrentNode()->info.first << '\n';
  
       cout << "trece_111\n";
       //TElem elem2;
       // try { elem2 = li.getCurrent(); }
       // catch (exception& e) { cout << " e de la ei\n"; }
       
       TElem elem2 = li.getCurrent();
       
       cout << "elem2 = " << elem2.first << " " << elem2.second << '\n';
        cout << "trece_222\n";
    ...
    li.next(); //2
    cout<<li.getCurrent().first<<endl;

    li.next(); //3   afiseaza tot 1
    cout<<li.getCurrent().first<<endl;
...
void testSearch(Relation r) {
   ...
   /* for (int i = cMax + 1; i < cMax + intervalDim; i++) {    MERGE
        assert(sm.search(i) == NULL_TVALUE);
    }*/
}
...
void testIterator(Relation r) {
    ...
    assert(itSM.valid());           // aici nu cumva era `assert(!itSM.valid())` ??
    itSM.first();
    assert(itSM.valid());
