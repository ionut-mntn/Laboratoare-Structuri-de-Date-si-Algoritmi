...
// using std::make_pair; // obs: cred ca pentru asta era dat deja inclusa libraria `utility` !!
...
Map::Map() {
    ...
    Cu toate acestea, nu sunt sigur daca am facut bine ca l-am lasat asa de mare; dar, oricum, am tot probat cu diferite valoari ale lui `m`, 
...
TValue Map::add(TKey c, TValue v){
    ...
    TValue rez = search(c);
	if(rez != NULL_TVALUE)      // daca, intr-adevar, exista cheia in container
    {	    
        TValue prev; // previous value
	 
        /*
        acestea este:
        1) fie in prima tabela hash pe pozitia data de functia `h1`
        2) fie in a doua tabela hash pe pozitia data de functia `h2`
        */
        ...
        return prev;            // la final, voi returna valoarea precedenta asociata cheii gasite in container, conform specificatie din interfata data
	}
    ...
    int poz1 = h1(c);          // obtin hashwert-ul elementului de inserat
    ...
    else        // altfel, continuam "eviction chain-ul"
    {
                    ...
                    Erau mai multe conditii de verificat si se complica conditia pentru `while`. Am lasat while(true) */
        {
            ...
            */
	        ...
	        int poz2 = h2(aux.first);       /* Calculam pozitia pe care ar trebui inserat elementul scos din prima ta */
            
            swap(aux, ht2[poz2]);   // The removed element is placed where calculated,
	                                                                // and we save the former element, in case it was removed.
            if (aux == NULL_TELEM) return NULL_TVALUE;
