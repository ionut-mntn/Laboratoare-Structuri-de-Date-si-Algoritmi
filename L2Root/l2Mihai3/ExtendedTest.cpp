...
int populate(SortedIndexedList& list, int cMin, int cMax, int s, int m) {
	...
	for (int i = 0; i < n; i++) {
		...
		list.add(v[i]);
		if (i == 8)
			cout << "get element la 12" << list.getElement(i - 1) << std::endl;
	}
	return n;
}
...
void testAddAndSearch(Relation r) {
	cout << "Test add and search" << r << endl;
	...
	/*
	cout << "ajunge" << '\n';
	for (int i = 1; i < d; i++) cout << list.getElement(i) << " ";
	cout << "ajunge" << '\n';
	*/
	...
	while (it.valid())
	{
		nr ++;
		try {
			TComp current = it.getCurrent(); cout << current << '\n';
		}
		catch(exception&){ cout << "La a " << nr <<" - a iteratie1"<< '\n'; cin.get();
		}
		try { it.next(); cout << "La a " << nr << " - a iteratie2" << '\n';  cin.get();
		}
		catch (exception&) { cout << "La a " << nr << " - a iteratie3" << '\n'; cin.get(); }
	}
	cout << " iese" << '\n';
	while (it.valid()) {
		cout << "trece" << '\n';
