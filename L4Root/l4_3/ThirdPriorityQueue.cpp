...
Element ThirdPriorityQueue::top () const
{
	...
	// ThirdPriorityQueue aux
	/*
	for(int i = 2; i <=len; i ++)
	//	if(elems[].second != elems[].second)
	*/

/*
	// Element aux1 = pop(), aux2 = pop(); // nu pot face astea pentru ca e const functia

	Element to_be_returned = elems[0];
	push(aux2);	// nu conteaza ordinea aici
	push(aux1);
*/
	return NULL_ELEMENT;
}
...
void ThirdPriorityQueue::bubble_down(int p)
{
	...
	while (poz < len)
	{
		...
		if (relChild != -1 && rel(elems[relChild].second, elem.second))
		...
		else
		{
			elems[poz] = elem;
			poz = len + 1;
			// interesant! facem asta cu `poz = len + 1` sa oprim while ul; 
...
// nu face parte din niciun namespace da nu i nimic
/*
int log_n(int len)
{
	int nr = 0; 
	while (len) { nr++; len /= 2; } // obtin parte intreaga din log n
}

void spatiere(int nr)
{
