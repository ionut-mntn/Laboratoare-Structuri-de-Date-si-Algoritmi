...
int SortedIndexedList::search(TComp e) const {
	...
	while(currentNode != nullptr)
	{
		...
		else if (rel(e, currentNode))
		{
			...
			// index--;
//			index = currentNode->LeftChildrenCount + 1;
		...
		else
		{
			...
			// int aux = currentNode->LeftChildrenCount + 1;
			int aux = currentNode->LeftChildrenCount;
			currentNode = currentNode->rc;
			//index += aux + currentNode->LeftChildrenCount;
			...
			// ajuta nr de copii din partea stanga
		}
	}
	//if (root == e) return index; // in caz ca am numai radacina !! ( cred ca totusi puteam sa las conditia while ului a doua varianta, fara nicio problema
	return -1;
	//return 0;
}
