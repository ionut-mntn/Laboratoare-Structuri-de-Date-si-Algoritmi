...
//	std :: cout<<"aiciiiii:"<<it.getCurrent()<<'\n';
	try {
		it.getCurrent();
		assert(false);
	}
...
void testAdd() {
	...
	iverif = 0;
	while (it2.valid()) {
            ///std :: cout << "acu:"<<it2.getCurrent() << '\n';
		vverif[iverif++] = it2.getCurrent();
		it2.next();
	}
///		cout << "TRECEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"<<'\n';
///	std :: cout << " ba man a ajuns AICIIII"<<'\n';
	for(int i = 0 ; i < 6; i++)
        cout << vverif[i] << " ";
    cout<<'\n';
	assert((vverif[0] == 5) && (vverif[1] == 4) && (vverif[2] == 3) && (vverif[3] == 2) && (vverif[4] == 1) && (vverif[5] == 0));
	///std :: cout << " ba man a ajuns AICIIII22222"<<'\n';
